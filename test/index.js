...
diorama.registerScenario("Alice can create a proposal and retrieve it", async (s, t, { alice }) => {
  ...
  const addr = await alice.callSync("matchmaking", "create_proposal", {message : "sup", timestamp: 1568067450})
  console.log(addr)
  ...
  const proposals = await alice.callSync("matchmaking", "get_proposals", {})
...
diorama.registerScenario("Bob can see the proposal created by Alice", async (s, t, { alice, bob }) => {
  ...
  const proposals = await bob.callSync("matchmaking", "get_proposals", {})
  console.log(proposals)
